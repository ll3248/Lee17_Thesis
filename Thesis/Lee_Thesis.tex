...
\begin{document}
  ...
  largely from C. Butts et al. (2015).
  ...
  graph with no edges or all possible edges from the \(G(N_{V}, p)\)
  model.
  ...
  see this, observe that the probability, \(p_k\), of a vertex connected
  \(k\) other vertices (without loops) is:
  ...
  \[e^{ln((1 - p)^{N_{V} - 1 - k})} = e^{N_{V} - 1 - k}ln(1 - \frac {c} {n - 1}) \simeq e^{-(n - 1 - k)(\frac {c} {n - 1})} \simeq e^{-c}\]
  ...
  randomly that connect vertices that did not have an edge before.
  ...
  number of edges or triangles of a graph in question. The denominator
  ...
  Adding a mapping; scripted Y is the ensemble, Y is random variable, y is
  the specific matrix Estimating thetas Maximum likelihood And MCMC
  ...
  Suppose we have a graph \(G\) and the only statsitic we have is L(G),
  the number of edges in G. Our probability model is thus
  
  \[P_{\theta, \mathcal{G}}(g) = \frac {exp(\theta_{L}L(g))} {\sum_{g' \in \mathcal{G}}^{} exp(\theta_{L}L(g))}, g \in \mathcal{G} \]
  Consider the probability distribution for the Erdős-Rényi model, as seen
  below.
